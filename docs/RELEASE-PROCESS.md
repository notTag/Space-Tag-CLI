...
Canonical release process for **space-labels** (the OSS CLI + configs) and how
the closed-source **SpaceTag** app consumes a release.

> This file lives in `notTag/osx-space-labels` at `docs/RELEASE-PROCESS.md` and is
> symlinked into the SpaceTag repo as `RELEASE-PROCESS.md`. Edit it here — both
> repos read the same source of truth.

Decoupled cadence: space-labels tags on its own schedule; SpaceTag bumps its pin
...
## Part 1 — Cut a space-labels release (this repo)
...
3. The `release` workflow (`.github/workflows/release.yml`) fires on `v*` and:
   - packs `space-labels-X.Y.Z.tar.gz` (`tar czf … sketchybar yabai zsh`)
   - writes `space-labels-X.Y.Z.tar.gz.sha256` (`sha256sum`)
...
In the **SpaceTag** repo (`space-labels-app`):

1. Edit `space-labels.lock`:
   ```sh
   SPACE_LABELS_VERSION=X.Y.Z
   SPACE_LABELS_SHA256=<sha256 from the release's .sha256 asset>
   ```
...
2. `Scripts/vendor.sh` reads the lock and stages configs into `Resources/config/`:
   - `gh release download vX.Y.Z -R notTag/osx-space-labels` the tarball + `.sha256`
   - **two-stage verify** — the release's own `.sha256`, then the audited
     `SPACE_LABELS_SHA256` from the lock. Mismatch = fail closed, nothing staged.
   - Fallback: while the lock is unpinned (`SPACE_LABELS_SHA256=PENDING_FIRST_RELEASE`)
     or `SPACE_LABELS_SOURCE=local`, it uses the `vendor/space-labels` submodule
     instead (dev loop).
...
```
space-labels:  commit → tag vX.Y.Z → CI publishes tarball + sha256
        │  (decoupled — own cadence)
        ▼
SpaceTag:  edit space-labels.lock (version + sha256) → PR → review
...
SpaceTag's lock currently ships `SPACE_LABELS_VERSION=0.0.0` /
`SPACE_LABELS_SHA256=PENDING_FIRST_RELEASE`, so `vendor.sh` is on the submodule
fallback. To switch onto the verified release path:

1. Cut `v0.1.0` here (Part 1).
2. Copy the published sha256 into `space-labels.lock` and set the version (Part 2).
